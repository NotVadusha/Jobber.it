"""One LLM call, four vendors — schema-validated JSON out, or RuntimeError.
Three transports (Anthropic, OpenAI-compatible, Ollama), one cached client each."""
...
class Spec:
    model: str
    env_key: str
    base_url: str | None = None  # OpenAI-compatible vendors only
    reasoning: bool = False  # takes an effort level; the rest reject the param
...
NUM_CTX = 8192
OLLAMA_TIMEOUT = 300.0  # a 3B on CPU under concurrency is slow, not hung
OLLAMA_429_TRIES = 5
...
def _client(provider: str):
    ...
    if provider == "ollama":
        # `key` is the base url here; accept it with or without the /v1 shim path.
...
def _anthropic(client, system: str, user: str, schema: type[BaseModel], model: str,
    ...
    response = client.messages.create(
        ...
        system=[
            # Cached, so keep this block byte-identical between calls.
...
def _ollama(client, system: str, user: str, schema: type[BaseModel], model: str) -> str:
    for attempt in range(OLLAMA_429_TRIES):
        ...
        if response.status_code != 429:
            break
        # Retry-After when the proxy sends one, exponential otherwise.
...
def call(
    ...
    for _ in range(2):
        ...
        except Exception as e:  # transport, rate limit, refusal
