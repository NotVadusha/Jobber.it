...
test('shows factual welcome counts and the real newest page without browse-only lies', async ({ page }) => {
  ...
  await expect(results.getByRole('button', { name: /save/i })).toHaveCount(20)
  await expect(results.getByRole('link').first()).toHaveAttribute('href', /^#\/job\//)
  await expect(results.locator('a[href^="http"]')).toHaveCount(0)
