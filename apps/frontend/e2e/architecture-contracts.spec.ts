...
test('an example submits its own query instead of the current draft', async ({ page }) => {
  let body: unknown
  await page.route('**/api/search', async (route) => {
    body = route.request().postDataJSON()
    await route.fulfill({ status: 200, contentType: 'application/json', json: searchWire })
  ...
  await page.goto('/')
  await page.getByRole('textbox', { name: 'Query' }).fill('a different draft')
  await page.getByRole('button', { name: 'python aws terraform', exact: true }).click()

  await expect(page.getByText('Nothing cleared the filters.')).toBeVisible()
  await expect(page.getByRole('textbox', { name: 'Query' })).toHaveValue('python aws terraform')
  expect(body).toMatchObject({ query: 'python aws terraform' })
})

test('editing and clearing individual filters preserves the other draft values', async ({ page }) => {
  let body: unknown
  await page.route('**/api/search', async (route) => {
    body = route.request().postDataJSON()
    await route.fulfill({ status: 200, contentType: 'application/json', json: searchWire })
  ...
  await page.getByRole('spinbutton', { name: 'max yrs' }).fill('0')
  await page.getByRole('spinbutton', { name: 'min $' }).fill('90000')
  await page.getByRole('spinbutton', { name: 'min $' }).fill('')
  await page.getByRole('button', { name: 'remote', exact: true }).click()
  ...
  expect(body).toMatchObject({
    query: 'postgres',
    filters: { remote_policy: ['hybrid'], seniority: ['senior'], experience_years: 0, min_salary: null },
  })
})

test('profile errors clear after a valid attachment or an explicit search', async ({ page }) => {
  await page.route('**/api/search', async (route) => {
    await route.fulfill({ status: 200, contentType: 'application/json', json: searchWire })
  })
  await page.goto('/')
  const file = page.getByLabel(/Attach a CV/)
  const emptyProfile = { name: 'empty.txt', mimeType: 'text/plain', buffer: Buffer.from('   ') }
  await file.setInputFiles(emptyProfile)
  await expect(page.getByRole('alert')).toContainText('no extractable text')
  await file.setInputFiles({
    name: 'profile.txt', mimeType: 'text/plain', buffer: Buffer.from('Python experience'),
  })
  await expect(page.getByRole('alert')).toHaveCount(0)
  await expect(page.getByText('profile.txt')).toBeVisible()
