...
const fetchPostingLookup = async (
  ids: readonly string[],
  signal?: AbortSignal,
...
export const isPostingNotFound = (error: unknown): boolean => {
  return error instanceof ApiError && error.code === 'POSTING_NOT_FOUND'
}

export const usePostingDetailQuery = (postingId: string) => {
  return useQuery({
...
export const usePostingLookupQuery = (ids: readonly string[]) => {
